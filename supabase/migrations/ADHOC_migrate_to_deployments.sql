-- ADHOC MIGRATION: Upgrade existing DB from production_incidents to deployments. Run once, then delete.
-- For fresh installs: use 016_deployments.sql instead.

-- Drop old table and RPC function
DROP TABLE IF EXISTS production_incidents CASCADE;
DROP FUNCTION IF EXISTS execute_incidents_query(TEXT);

-- Create deployments table (same as 016_deployments.sql Part 2)
...
-- Seed 15 deployments (same as 016_deployments.sql Part 3)
...
-- RPC function for safe SQL execution on deployments table
